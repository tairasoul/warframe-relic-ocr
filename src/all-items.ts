...
import items, { Relic } from "warframe-items";
...
const itms = new items({category: ["Relics"]}) as Relic[];
...
type item_info = {
    ...
    id: string;
    median_price: number;
    highest_order: order | undefined;
    relics: { [relic_name: string]: number };
}

const rewards: item_info[] = [];

async function processRelic(relic: string) {
    const rlic = itms.find((v) => v.name == relic);
    if (rlic) {
        console.log(`processing ${relic}`);
        for (const reward of rlic.rewards as reward[]) {
            await processRewards(relic, reward);
        }
    }
}

async function processRewards(relic: string, reward: reward) {
...
function parseInfo(iteminfo: item_info) {
    let str = "";
...
function parseRelicInfo(info: { [relic_name: string]: number }) {
